...
namespace BansheeGz.BGSpline.Curve
{
	...
	public class BGCurveBaseMath : BGCurveMathI, IDisposable
	{
		...
		public BGCurveBaseMath(BGCurve curve, bool traceChanges, int parts = 30, bool usePointPositionsToCalcTangents = false) : this(curve, new Config(Fields.Position) { Parts = parts, UsePointPositionsToCalcTangents = usePointPositionsToCalcTangents }) { }
		...
		public virtual void Recalculate(bool force = false)
		{
			...
			if (currentSectionsCount != sectionsCount)
			{
				if (currentSectionsCount < sectionsCount)
				{
					...
					if (toAddCount > 0)
						for (var i = 0; i < toAddCount; i++) cachedSectionInfos.Add(new SectionInfo());
				}
				else
				{
					//too many sections
