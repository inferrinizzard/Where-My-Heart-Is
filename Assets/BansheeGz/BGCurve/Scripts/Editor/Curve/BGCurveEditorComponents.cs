	...
	public class BGCurveEditorComponents : BGCurveEditorTab
	{
		...
		public override void OnInspectorGui()
		{
			...
			if (length > 0)
			{
				...
				EditorGUILayout.HelpBox(
					...
					"\r\n" +
					"\r\n" +
					"Components allows to add functionality without any scripting.", MessageType.Info);
		...
		internal sealed class Tree : BGTreeView<Tree.CcNode>
		{
			...
			public void Refresh(BGCc[] components = null, bool force = false)
			{
				...
				if (Roots.Count > 0)
					foreach (var root in Roots) root.FillState(instanceId2Collapsed);
				...
				try
				{
					...
					foreach (var list in type2NodeList.Values)
						foreach (var node in list)
							if (!node.Processed) node.ProcessStructure();
				}
