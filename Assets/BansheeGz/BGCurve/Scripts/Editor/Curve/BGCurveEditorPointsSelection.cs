	...
	public class BGCurveEditorPointsSelection
	{
		...
		public void InspectorSelectionOperations()
		{
			BGEditorUtility.VerticalBox(() =>
			{
				...
				BGEditorUtility.HorizontalBox(() =>
				{
					...
					if (BGEditorUtility.ButtonWithIcon(BGBinaryResources.BGDelete123, "Delete selected points"))
		...
		private sealed class PointsContainer : BGCurvePointI
		{
			...
			public void UpdateFields()
			{
				...
				if (name2Value.Count > 0)
					foreach (var key in name2Value.Keys) existingKeys.Add(key);
				...
				if (existingKeys.Count > 0)
					foreach (var key in existingKeys) name2Value.Remove(key);
			}
		}
	}
}
