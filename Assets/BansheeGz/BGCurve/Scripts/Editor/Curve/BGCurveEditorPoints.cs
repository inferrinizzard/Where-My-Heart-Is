...
namespace BansheeGz.BGSpline.Editor
{
	...
	public class BGCurveEditorPoints : BGCurveEditorTab
	{
		...
		private void InspectorTopSection()
		{
			if (Curve.PointsCount == 0)
				EditorGUILayout.HelpBox(
					...
					"\r\n" +
					"\r\n5) Ctrl + LeftClick over existing spline to insert a point", MessageType.Info);
		...
		{
			if (pointTransformTrackers.Count > 0)
				foreach (var tracker in pointTransformTrackers) tracker.Release();
