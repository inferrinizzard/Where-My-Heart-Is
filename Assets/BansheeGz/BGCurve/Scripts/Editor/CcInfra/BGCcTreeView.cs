...
namespace BansheeGz.BGSpline.Editor
{
	public class BGCcTreeView : BGTreeView<BGCcTreeView.CcNode>
	{
		...
		public BGCcTreeView(BGCurve curve, Type dependsOnType, bool ignoreExcludeFromMenuAttribute, Action<string> messageAction, Action<Type> typeWasChosenAction) : base(new Config(0, (int) (IconSize * .9f), (int) (IconSize * .5f), 2, 16))
		{
			Curve = curve;
		...
		public static Type[] GetAllSubTypes(Type targetType, Type excludeAttributeType)
		{
			...
			foreach (var assembly in assemblies)
				result.AddRange(from type in assembly.GetTypes() where type.IsClass where!type.IsAbstract where type.IsSubclassOf(targetType) where excludeAttributeType == null || BGReflectionAdapter.GetCustomAttributes(type, excludeAttributeType, true).Length == 0 select type);
		...
		public sealed class CcNode : BGTreeNode<CcNode>
		{
			...
			public CcNode(BGCcTreeView tree, CcData ccData, Action<Type> typeWasChosenAction) : base(tree)
			{
				this.ccData = ccData;
		...
		public sealed class CcData
		{
			...
			public CcData(Type type, bool single, BGCc.CcDescriptor ccDescriptor) : this(type, single, ccDescriptor.Name)
			{
