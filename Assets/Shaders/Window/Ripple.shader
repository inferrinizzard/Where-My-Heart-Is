	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float y = (1 / _ScaleY) * (i.uv.y - 0.5 - _TranslationY - _RippleOffset);
				//float distance = length((i.uv.xy - float2(0.5, 0.5) - float2(_TranslationX, _TranslationY));
				float distance = length(float2(x, y));

				float2 direction = normalize(i.uv.xy - float2(0.5, 0.5) - float2(_TranslationX, _TranslationY + _RippleOffset));
				//float angle = atan2(i.uv.x, i.uv.y);
				float heartOffset = _RippleOffset * heartEquation(direction.y, direction.x);//heart eqn here as function of i.uv.xy
				float diffFromOffset = abs(heartOffset - distance);
				//float diffFromOffset = _RippleOffset - sqrDist;
