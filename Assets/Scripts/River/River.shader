Shader "River" {
 Properties {
		...
		_Specs1X("Spec X Scale", float) = 1
		_Specs1Y ("Spec Y Scale", float) = 1

        [Header(Spec Layer 2)]
        _Specs2 ("Specs", 2D) = "white" {}
        _SpecColor2 ("Spec Color", Color) = (1,1,1,1)
        _SpecDirection2 ("Spec Direction", Vector) = (0, 1, 0, 0)
		_Specs2X("Spec X Scale", float) = 1
		_Specs2Y("Spec Y Scale", float) = 1
    ...
    SubShader {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
            ...
            float2 specCoordinates1 = IN.uv_Specs1 * float2(_Specs1X, _Specs1Y) + _SpecDirection1 * _Time.y ;//
            ...
            float2 specCoordinates2 = IN.uv_Specs2 * float2(_Specs2X, _Specs2Y) + _SpecDirection2 * _Time.y;
            fixed4 specLayer2 = tex2D(_Specs2, specCoordinates2) * _SpecColor2;// * float2(_Specs2X, _Specs2Y)
